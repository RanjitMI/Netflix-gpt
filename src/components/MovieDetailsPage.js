    ...
    <>
      ...
      <div className="absolute hidden md:block top-[0px] h-screen w-screen bg-gradient-to-r from-black">
        <div className="text-white absolute ml-14 top-[250px] ">
          ...
          <div className="flex ml-[-15px] mt-6">
            {movies?.genres?.map((genre) => {
              return (
                <p
                  key={genre?.id}
                  className="rounded-full ml-2 px-4 py-2 bg-gray-800 text-white"
                >
                  {genre?.name}
                </p>
              );
          ...
          <Link
            to="/browse"
            className="mt-6 px-6 py-2 bg-red-600 text-white rounded-full hover:bg-red-700 transition py-4"
          >
            Back to Browse
          </Link>
          </div>
